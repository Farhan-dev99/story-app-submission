...
export async function subscribe() {
  ...
  if (await isCurrentPushSubscriptionAvailable()) {
    errorAlert("Sudah berlangganan push notification.");
    return;
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      errorAlert(failureSubscribeMessage);
      // Undo subscribe to push notification
      await pushSubscription.unsubscribe();
      return;
    }
 
    successAlert(successSubscribeMessage);
  } catch (error) {
...
export async function unsubscribe() {
    ...
    try {
      ...
      if (!pushSubscription) {
        errorAlert('Tidak bisa memutus langganan push notification karena belum berlangganan sebelumnya.');
        return;
      }
      const { endpoint, keys } = pushSubscription.toJSON();
      const response = await unsubscribePushNotification({ endpoint });
      if (!response.ok) {
        errorAlert(failureUnsubscribeMessage);
      ...
      successAlert(successUnsubscribeMessage);
    } catch (error) {
      errorAlert(failureUnsubscribeMessage);
